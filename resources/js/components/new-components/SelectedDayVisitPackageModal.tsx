...
const styles = `
  ...
  .footer-send-btn.whatsapp {
    background: #25d366; color: #fff;
    box-shadow: 0 4px 14px rgba(37,211,102,0.3);
  }
  .footer-send-btn.whatsapp:hover { box-shadow: 0 6px 20px rgba(37,211,102,0.45); transform: translateY(-1px); }
  .footer-send-btn.email {
    background: #fff; color: #5a3e2b;
    border: 1.5px solid rgba(184,146,75,0.35);
    box-shadow: 0 2px 8px rgba(0,0,0,0.06);
  }
  .footer-send-btn.email:hover { background: rgba(255,251,240,0.9); border-color: rgba(184,146,75,0.6); }
  .footer-send-btn .btn-label-desktop { display: inline; }
  .footer-send-btn .btn-label-mobile { display: none; }
  @media (max-width: 500px) {
    .footer-send-btn .btn-label-desktop { display: none; }
    .footer-send-btn .btn-label-mobile { display: inline; }
    .footer-send-btn { padding: 10px 13px; font-size: 0.8rem; }
  }
  .footer-enquiry-hint {
    font-size: 0.78rem; color: #9a7d5a; margin-bottom: 10px;
...
export default function SelectedDayVisitPackageModal() {
    ...
    const getDayVisitMessage = () => {
        const title = selectedDayVisitPackageItem?.title || 'Day Visit Package';
        const date = visitDate ? formatDate(visitDate) : 'Not selected';
        const total = dvTotal;

        return `🏰 *Tafaria Castle – Day Visit Enquiry*
...
Please confirm availability. Thank you! 🙏`;
    ...
    const handleDvSendWhatsApp = () => {
        const url = `https://wa.me/+254708877244?text=${encodeURIComponent(getDayVisitMessage())}`;
        window.open(url, '_blank');
    };

    const handleDvSendEmail = () => {
        const subject = `Day Visit Enquiry – ${selectedDayVisitPackageItem?.title || 'Tafaria'}`;
        const body = getDayVisitMessage().replace(/\*/g, '');
        window.open(
            `mailto:info@tafaria.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`,
            '_blank',
        );
    };
    return (
                    ...
                    {showDayVisitModal && selectedDayVisitPackageItem && (
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    <div className="footer-enquiry-hint">
                                        💬 Prefer to enquire first? Send your
                                        details via WhatsApp or Email below.
                                    </div>
                                    <div
                                        style={{
                                            display: 'flex',
                                    ...
                                    >
                                        ...
                                        >
                                            <motion.button
                                                className="footer-send-btn email w-full"
                                                onClick={handleDvSendEmail}
                                                whileHover={{ scale: 1.02 }}
                                                whileTap={{ scale: 0.97 }}
                                            >
                                                <Mail size={14} />
                                            ...
                                            <motion.button
                                                className="footer-send-btn whatsapp w-full"
                                                onClick={handleDvSendWhatsApp}
                                                whileHover={{ scale: 1.02 }}
                                                whileTap={{ scale: 0.97 }}
                                            >
                                                <svg
