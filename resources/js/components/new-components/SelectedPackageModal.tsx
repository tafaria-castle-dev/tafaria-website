...
export default function SelectedPackageModal() {
    ...
    const getBookingMessage = () => {
        const packageTitle = selectedPackage?.title || 'Unknown Package';
        const dateIn = checkIn ? formatDate(checkIn) : 'Not selected';
        const dateOut = checkOut ? formatDate(checkOut) : 'Not selected';

        return `🏰 *Tafaria Castle – Booking Enquiry*

Package: ${packageTitle}
Check-in: ${dateIn}
Check-out: ${dateOut}
Duration: ${pkgNights} night${pkgNights !== 1 ? 's' : ''}

Guests:
- Adults: ${numAdults}
- Children (4–11): ${numKids4to11}
- Infants (0–3): ${numKids0to3}
- Total: ${numAdults + numKids4to11 + numKids0to3}
...
Please confirm availability and pricing. Thank you! 🙏`;
    ...
    const handleSendWhatsApp = () => {
        ...
        const url = `https://wa.me/${phone}?text=${encodeURIComponent(getBookingMessage())}`;
        window.open(url, '_blank');
    };

    const handleSendEmail = () => {
        const email = 'info@tafaria.com';
        const subject = `Booking Enquiry – ${selectedPackage?.title || 'Tafaria Package'}`;
        const body = getBookingMessage().replace(/\*/g, '');
        window.open(
                                    ...
                                    {packageStep === 'dates' ? (
                                        <>
                                            <div className="footer-enquiry-hint">
                                                💬 Prefer to enquire first? Send
                                                your details via WhatsApp or
                                                Email below.
                                            </div>
                                            <div
                                                style={{
